      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/92 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <Button asChild variant="outline" className="rounded-lg border-border/70 bg-card/60 text-foreground">
      ...
      <section className="relative isolate overflow-hidden border-b border-border/60">
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-73px)] max-w-7xl gap-12 px-5 py-16 sm:px-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,420px)] lg:px-8 lg:py-24">
          <div className="flex flex-col justify-center">
            <Badge variant="outline" className="mb-6 w-fit rounded-md border-primary/50 bg-card/60 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-primary">
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              ...
              <Button asChild size="lg" variant="outline" className="rounded-lg border-primary/40 bg-card/60 text-foreground">
                <Link to="/auth">
                  <Lock />
                  Episode 1 + materials
                </Link>
            ...
            <ul className="mt-12 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {keyFacts.map((fact) => (
                <li key={fact.label} className="rounded-lg border border-border/60 bg-card/70 p-4 backdrop-blur-sm">
            ...
            <Card className="w-full rounded-lg border-primary/20 bg-card/88 shadow-2xl backdrop-blur-sm">
              ...
              <CardContent className="space-y-5 text-sm leading-6 text-muted-foreground">
                <div className="flex items-start gap-3">
                  ...
                  <p>
                    Public visitors can view the logline, pitch imagery, team, creator bios, contact,
                    and trailer placeholders.
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <Lock className="mt-0.5 text-primary" />
                  <p>
                    Approved viewers get access to Episode 1, the pitch deck, one-sheet, series bible,
                    and pilot script inside the private library.
                  </p>
                </div>
                <div className="flex items-start gap-3">
        ...
        <div className="undone-wrap grid gap-10 lg:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)]">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">
              The pitch positions UNDONE as a sharp, emotionally literate series: a love story that is
              not only about chemistry, but about what it really takes to stay present once the fantasy
              burns off.
            </p>
            <div className="mt-8 space-y-4">
              {documents.map((item) => (
                <div key={item.title} className="flex items-start gap-3 rounded-lg border border-border/60 bg-card/55 p-4">
      ...
      <section id="trailers" className="undone-section">
        <div className="undone-wrap">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-3xl font-semibold text-foreground sm:text-5xl">Trailer-ready public experience</h2>
              <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground sm:text-lg">
                The public-facing media area is live structurally now. Final trailer files can be dropped
                into these slots later without changing the page architecture.
              </p>
            </div>
          </div>
              ...
              <Card key={slot.title} className="rounded-lg border-border/70 bg-card/65 shadow-lg">
                <CardHeader>
                  <CardTitle className="flex items-center justify-between gap-4 text-xl text-foreground">
                    ...
                    <Badge variant={slot.publicAccess ? "outline" : "default"} className="rounded-md">
                ...
                <CardContent>
                  <div className="flex aspect-video items-center justify-center rounded-lg border border-dashed border-primary/40 bg-background/60">
                    <div className="text-center">
                      ...
                      <p className="mt-3 text-sm font-medium text-foreground">Upload-ready player shell</p>
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="mt-8 grid gap-5">
              {reasonsToWatch.map((reason) => (
                <div key={reason.title} className="rounded-lg border border-border/60 bg-card/55 p-5">
      ...
      <section className="undone-section">
        <div className="undone-wrap">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-3 text-3xl font-semibold text-foreground sm:text-5xl">Ten episodes that escalate intimacy, fracture, and reckoning.</h2>
          </div>
          <div className="mt-10 grid gap-5 lg:grid-cols-2">
            {episodes.map((episode) => (
              <Card key={episode.title} className="rounded-lg border-border/70 bg-card/55 shadow-lg">
                <CardHeader className="pb-4">
      ...
      <section id="team" className="undone-section">
        <div className="undone-wrap grid gap-10 lg:grid-cols-[minmax(0,0.86fr)_minmax(0,1.14fr)]">
          ...
          <div>
            ...
            <h2 className="mt-3 text-3xl font-semibold text-foreground sm:text-5xl">A young, bold team with full-female creative leadership.</h2>
            <div className="mt-8 grid gap-5">
              {leadership.map((person) => (
                <Card key={person.name} className="rounded-lg border-border/70 bg-card/55 shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-2xl text-foreground">{person.name}</CardTitle>
                    <p className="text-sm uppercase tracking-[0.14em] text-primary">{person.role}</p>
                  </CardHeader>
                  <CardContent>
      ...
      <section id="contact" className="undone-section pb-24">
        <div className="undone-wrap grid gap-10 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
          <div className="rounded-lg border border-border/60 bg-card/55 p-6 sm:p-8">
            ...
            <h2 className="mt-3 text-3xl font-semibold text-foreground sm:text-5xl">Request the full private library.</h2>
            <p className="mt-6 text-base leading-7 text-muted-foreground sm:text-lg">
              Approved viewers receive access to the Episode 1 screening room and the protected
              document library. Until final media is uploaded, the secure structure and approval flow
              are in place and ready to receive those files.
            </p>
          ...
          <div className="rounded-lg border border-border/60 bg-card/55 p-6 sm:p-8">
            ...
            <h2 className="mt-3 text-3xl font-semibold text-foreground sm:text-5xl">Let’s make television history.</h2>
            <div className="mt-8 space-y-5 text-base leading-7 text-foreground sm:text-lg">
              ...
              <div className="flex items-start gap-4">
                ...
                <a className="transition-colors hover:text-primary" href={contact.website} target="_blank" rel="noreferrer">
                  {contact.website}
                </a>
              </div>
              <Separator className="bg-border/60" />
